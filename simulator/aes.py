#!/usr/bin/env python
#
# Licensed by "The MIT License". See file LICENSE.
#
# Implementation of AES-128 with the option to simulate fault
# injections in round 8.
# expk are already expanded keys reconstructed from the fault analysis.
#
#Rijndael S-box
sbox = [
 ]

# Rijndael Inverted S-box
inv_sbox = [
 ]

# Rijndael Rcon
rcon = [
 ]
from typing import List
from copy import deepcopy
class SBOX:
    def __init__(self,data:List[int]) -> None:
        self.data = data
        self.sbox_len = len(data)
    def __check_key(self,key:int):
        if not key >=0 and key < 2**len(self.sbox_len):
            raise(f"key size is not match, key must of key bit {len(self.sbox_len)}")
    def __getBin(self,key:int)->List[int]:
        ret:List[int] = list()
        for _ in range(self.sbox_len):
            bin_digit = key % 2
            key = key // 2
            ret.append(bin_digit)
        return ret[::-1]  
    def __swap(self,idx:int,forw:bool,sbox:List[int]):
        swap_idx = 0
        if forw:
            swap_idx = (idx + idx + 1) % self.sbox_len
        else:
            swap_idx = (idx + idx + 1) % self.sbox_len
            # print(idx+1 % self.sbox_len)
            if (idx+1) % self.sbox_len == 0:
                swap_idx = (swap_idx + 1) % self.sbox_len
        # print(f"index: {idx} and {swap_idx}")
        sbox[idx],sbox[swap_idx]  = sbox[swap_idx],sbox[idx]

    def debug(self,idx:int,forw:bool,sbox:List[int]):
        return self.__swap(idx,forw,sbox)
    def get(self,key:int)->List[int]:
        self.__check_key(key)
        ret:List[int] = deepcopy(self.data)
        list_key:List[int] = self.__getBin(key)
        idx = None
        if list_key[0] == 1:
            # BACKWARD
            # ret = ret[::-1]
            self.__swap(0,False,ret)
            # ret = ret[::-1]
            idx = 1             
            for key_bin in list_key[1:]:
                if key_bin:
                    # ret = ret[::-1]
                    self.__swap(idx,False,ret)
                    # ret = ret[::-1]
                idx += 1 
        else:
            # FORWARD
            idx = 0             
            for key_bin in list_key[1:][::-1]:
                if key_bin:
                    ret = ret[::-1]
                    self.__swap(idx,True,ret)
                    ret = ret[::-1]
                idx += 1
        return ret
def generate_inverse_sbox(sbox):
    inv_sbox = [0] * len(sbox)
    for i in range(len(sbox)):
        inv_sbox[sbox[i]] = i
    
    return inv_sbox
def cvt_8bit_bin(num:int):
    ret = []
    for _ in range(8):
        ret.append(num%2)
        num = num // 2
    return ret[::-1]
transpose = lambda _sbox_bin:  [[_sbox_bin[j][i] for j in range(len(_sbox_bin))] for i in range(len(_sbox_bin[0]))]
cvt_bin_8bit = lambda _val_list: sum([val * (2**idx) for idx, val in enumerate(_val_list[::-1])])
sbox_bin = [cvt_8bit_bin(v) for v in sbox]
t_sbox_bin = transpose(sbox_bin)
__sbox = SBOX(t_sbox_bin)
def encrypt(p,k=None,expk=None,fault=0x00,floc=0):
  _k = k & (2**4 - 1)
  _sbox = [cvt_bin_8bit(i) for i in transpose(__sbox.get(_k))]
  _invsbox = generate_inverse_sbox(_sbox)
  plaintext = to_matrix(p)
  if expk == None:
    expanded_key = expandkey(k)
  else:
    expanded_key = expk
  state = add_roundkey(plaintext, get_roundkey(expanded_key,0))
  for r in range(1,10):
    if (r == 8):
      state[ floc % 4 ][ floc // 4 ] ^= fault # fault injection
    state = add_roundkey( mixcolumns(shiftrows( subbytes(state,get_roundkey(expanded_key,r),_sbox))), get_roundkey(expanded_key,r))
  # final round
  return to_bitstring( add_roundkey(shiftrows( subbytes(state,get_roundkey(expanded_key,10),_sbox)), get_roundkey(expanded_key,10)))

def decrypt(c,k):
  _k = k & (2**4 - 1)
  _sbox = [cvt_bin_8bit(i) for i in transpose(__sbox.get(_k))]
  _invsbox = generate_inverse_sbox(_sbox)
  ciphertext = to_matrix(c)
  expanded_key = expandkey(k)
  state = inv_subbytes(inv_shiftrows( add_roundkey(ciphertext, get_roundkey(expanded_key,10))),get_roundkey(expanded_key,10),_invsbox)
  for r in range(9,0,-1):
    roundKey = get_roundkey(expanded_key,r)
    state = inv_subbytes(inv_shiftrows(inv_mixcolumns( add_roundkey(state,roundKey))),roundKey,_invsbox)
  return to_bitstring(add_roundkey(state, get_roundkey(expanded_key,0)))

def add_roundkey(state,roundKey):
  for i in range(0,4):
    for j in range(0,4):
      state[i][j] ^= roundKey[i][j]
  return state

def get_roundkey(expanded_key,r):
  key = 0
  for i in range(4):
    key ^= expanded_key[4*r+i] << 96-32*i
  return to_matrix(key)

def ks_core(t,r):
  b = ((t << 8) & 0xffffffff) ^ (t >> 24)
  c = rcon[r] << 24
  for i in range(4):
    c ^= sbox[(b >> 24 - 8*i) & 0xff] << 24 - 8*i
  return c

def expandkey(key):
  Nb = 4
  Nk = 4
  Nr = 10
  rconi = 1
  expanded_key = [(key >> 96-32*i) & 0xffffffff for i in range(4)]
  for i in range(Nk,Nb * (Nr + 1)):
    t = expanded_key[i-1]
    if (i % Nk == 0):
      t = ks_core(t,rconi)
      rconi += 1
    expanded_key.append(expanded_key[i-Nk] ^ t)
    i += 1
  return expanded_key

def subbytes(state,key,sbox):
  # print(f"KEY AT ENCRIPTION: {key}")
  return apply_subbytes(state,sbox,False)

def inv_subbytes(state,key,sbox):
  # print(f"KEY FROM DECRRIPTION: {key}")
  return apply_subbytes(state,sbox,True)

def apply_subbytes(state,_sbox,isInv):
  for i in range(0,4):
    for j in range(0,4):
      if (not isInv):
        state[i][j] = _sbox[state[i][j]]
      else:
        state[i][j] = _sbox[state[i][j]]
  return state

def shiftrows(state):
  return apply_shiftrows(state,False)

def inv_shiftrows(state):
  return apply_shiftrows(state,True)

def apply_shiftrows(state,isInv):
  for i in range(1,4):
    for j in range(1,i+1):
      if(not isInv):
        state[i].append(state[i].pop(0)) # left-shift
      else:
        state[i].insert(0,state[i].pop()) # right-shift
  return state

def mixcolumns(state):
  return apply_mixcolumns(state,False)

def inv_mixcolumns(state):
  return apply_mixcolumns(state,True)

def apply_mixcolumns(state,isInv):
  s = [
    [0x00,0x00,0x00,0x00],
    [0x00,0x00,0x00,0x00],
    [0x00,0x00,0x00,0x00],
    [0x00,0x00,0x00,0x00],
  ]
  if (not isInv):
    # matrix for mixcolumns
    m = [
      [0x02,0x03,0x01,0x01],
      [0x01,0x02,0x03,0x01],
      [0x01,0x01,0x02,0x03],
      [0x03,0x01,0x01,0x02]
    ]
  else:
    # matrx for invMixColumns
    m = [
      [0x0e,0x0b,0x0d,0x09],
      [0x09,0x0e,0x0b,0x0d],
      [0x0d,0x09,0x0e,0x0b],
      [0x0b,0x0d,0x09,0x0e]
    ]
  for i in range(0,4):
    for j in range(0,4):
      for k in range(0,4):
        s[i][j] ^= gm(state[k][j],m[i][k])
  return s

# galois multiplication
def gm(a,b):
  f = 0x11b  # AES polynomial: x^8 + x^4 + x^3 + x + 1
  g = 0
  for i in range(8):
    if (b & 1) == 1: g ^= a
    hbs = (a & 0x80) # check if highest bit is set
    a <<= 1
    if hbs == 0x80: a ^= f
    b >>= 1
  return g



def to_matrix(s):
  m = []
  for i in range(4):
    m.append([(s >> (120-8*(4*j+i))) & 0xff for j in range(4)])
  return m

def to_bitstring(s):
  m = 0
  for i in range(len(s)):
    for j in range(len(s[i])):
      m ^= s[i][j] << (120-8*(4*j+i))
  return m

def printmatrix(m):
  for i in range(4):
    for j in range(4):
      print("{:02x} ".format(m[i][j]),)
    print ("")
  print ("")

def printkeys(keys):
  for i in range(len(keys)):
    print ("{:08x}".format(keys[i]))
  print ("")


#----------------------------------------------------------------------------
# NO USE
def test_aes():

 # KEY, PLAINTEXT, CIPHERTEXT
  L = [(0x00000000000000000000000000000000,0x80000000000000000000000000000000,0x3ad78e726c1ec02b7ebfe92b23d9ec34),
       (0x00000000000000000000000000000000,0xc0000000000000000000000000000000,0xaae5939c8efdf2f04e60b9fe7117b2c2),
       (0x00000000000000000000000000000000,0xe0000000000000000000000000000000,0xf031d4d74f5dcbf39daaf8ca3af6e527),
       (0x00000000000000000000000000000000,0xf0000000000000000000000000000000,0x96d9fd5cc4f07441727df0f33e401a36),
       (0x00000000000000000000000000000000,0xf8000000000000000000000000000000,0x30ccdb044646d7e1f3ccea3dca08b8c0),
       (0x00000000000000000000000000000000,0xff800000000000000000000000000000,0xa81738252621dd180a34f3455b4baa2f),
       (0x00000000000000000000000000000000,0xffffffffffffffffffffe00000000000,0x9ca547f7439edc3e255c0f4d49aa8990),
       (0x2b7e151628aed2a6abf7158809cf4f3c,0x6bc1bee22e409f96e93d7e117393172a,0x3ad77bb40d7a3660a89ecaf32466ef97),
       (0x2b7e151628aed2a6abf7158809cf4f3c,0xae2d8a571e03ac9c9eb76fac45af8e51,0xf5d3d58503b9699de785895a96fdbaaf),
       (0x2b7e151628aed2a6abf7158809cf4f3c,0x30c81c46a35ce411e5fbc1191a0a52ef,0x43b1cd7f598ece23881b00e3ed030688),
       (0x2b7e151628aed2a6abf7158809cf4f3c,0xf69f2445df4f9b17ad2b417be66c3710,0x7b0c785e27e8ad3f8223207104725dd4)]

  for e in L:
    enc = encrypt(e[1],e[0])
    dec = decrypt(enc,e[0])
    if e[1] == dec:
       print("PASS")
    else:
       print("FAIL")
    # assert e[2] == encrypt(e[1],e[0])
    

  print("All tests passed.")

if __name__ == '__main__':
  test_aes()
